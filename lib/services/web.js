...
var WebService = function(options) {
  ...
  this.origins = options.origins || this.node.origins || '*:*';
...
WebService.prototype.start = function(callback) {
  ...
  this.io = socketio.listen(this.server,{origins: this.origins});
...
WebService.prototype.socketHandler = function(socket) {
  ...
  socket.on('subscribe', function(name, params) {
    log.info(anonymize(remoteAddress), 'web socket subscribe:', name);
    bus.subscribe(name, params);
  });

  socket.on('unsubscribe', function(name, params) {
    log.info(anonymize(remoteAddress), 'web socket unsubscribe:', name);
  ...
  socket.on('disconnect', function() {
    log.info(anonymize(remoteAddress), 'web socket disconnect');
    bus.close();
  });
};
...
WebService.prototype.transformHttpsOptions = function() {
  ...
  this.httpsOptions = {
    ...
    requestCert: this.httpsOptions.requestCert
  };
};
